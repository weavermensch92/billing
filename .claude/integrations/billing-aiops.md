# Integrations / Billing ↔ AI Observer — 데이터 파이프라인

> **I-004** — AI Observer 사용량 snapshot ↔ Billing 실결제 transactions 매칭 + Anthropic 패스스루 교차 검증 + MSP대행 billing model 진화.
...
| 제품 | 역할 | 데이터 소유 |
|---|---|---|
| **AI Observer** | AI 사용 **관측** (프롬프트·토큰·비용 추정) | `aiops.logs`, `aiops.usage_snapshots` |
| **Billing** | AI 서비스 **결제·청구** (실제 카드 승인) | `billing.transactions`, `billing.invoices` |

AI Observer 는 "누가 AI 를 얼마나 썼는가" → **추정 비용**
...
| 고객 조합 | 예시 | 통합 효과 |
|---|---|---|
| AI Observer 만 | AI 현황 보고 싶은 고객 | 추정 비용만 표시 |
| Billing 만 | 결제 관리만 원하는 고객 | 실결제 원장 |
| **AI Observer + Billing** | 풀 스택 관리 | 추정 vs 실제 비교 / 이상 감지 정확도 ↑ |

**핵심**: 한 고객사가 두 제품 모두 계약 시 `org_id` 공유 (동일 Gridge 조직).

## I-004-03. 데이터 파이프라인

### A. AI Observer → Billing 방향 (사용량 → 결제 매칭)

```
[AI Observer logs] 프롬프트·토큰 수집 (실시간)
      │
      ▼
[AI Observer usage_snapshots] 일일 집계 (03:00 배치)
      │
...
### B. Billing → AI Observer 방향 (실결제 → 사용량 검증)

```
[Billing transactions] 카드 승인 수신 (실시간)
      │
      ▼ (월말 배치 M+1일 02:00)
[교차 검증 엔진] AI Observer 추정 vs Billing 실결제 대조
      │
      ├─ 오차 ≤ 5% → OK (정상)
      ├─ 5% < 오차 ≤ 20% → 경고 (CSM 검토)
...
**Phase 0 (현재)**:
- AI Observer: 고객이 자기 API 키 (Mode C) / 그릿지 관리 키 (Mode A)
- Billing: 고객 VCN 결제 (재판매)
- 두 제품 별개 계약

**Phase 1 (v0.20+)**:
- `org_contracts.aiops_bridge_enabled = TRUE` 신규 플래그
- AI Observer 프록시를 거치는 모든 API 호출 → Billing transactions 자동 기록
- 고객은 **하나의 세금계산서** 로 AI Observer 비용 + 일반 AI 서비스 비용 통합 수신
- Anthropic API 만 패스스루, 나머지는 원가 재판매

**Phase 2 (향후)**:
- Billing 이 MSP 주 계약, AI Observer 는 "비용 분석 모듈" 로 내장
- 업셀 전환율 측정 표준화

## I-004-05. Anthropic 패스스루 교차 검증 (PB-007)

AI Observer `aiops.logs` 의 Anthropic API 호출 → Billing `transactions.is_anthropic_passthrough = TRUE` 일치 여부 검증:
...
**AI Observer 에서 급증 감지 + Billing 에서 미반영** 시나리오:
```
AI Observer: 지난 주 대비 토큰 사용 300% 증가
...
**Billing 에서 이상 결제 + AI Observer 로그 없음**:
```
Billing: 특정 서비스 이번 주 결제 +500% (₩2M → ₩12M)
AI Observer: 같은 서비스 사용량 변화 없음
      ↓
...
ALTER TABLE billing.transactions ADD COLUMN aiops_log_count INT;  -- 매칭된 AI Observer 로그 수
ALTER TABLE billing.transactions ADD COLUMN aiops_estimated_krw BIGINT;  -- AI Observer 추정
...
AI Observer 도입 시점 → Billing 도입 제안:
- AI Observer 고객의 월 추정 비용이 **₩500만 초과** 3개월 연속
- 즉, 법인카드 관리 복잡도 임계점 도달
- `upsell_signals INSERT (type='billing_upsell', source_product='aiops')`

Billing 도입 시점 → AI Observer 도입 제안:
...
- AI Observer 데이터는 고객 데이터 (프롬프트 포함) → **Billing 테이블에 혼합 저장 금지**
- Billing 에는 **추정 비용 수치만** 저장 (프롬프트 원문 X)
- 해지 시: AI Observer / Billing 각각 D+30 삭제 (서로 독립)

## I-004-10. 자동 검증 체크리스트

- [ ] AI Observer 가 Billing 의 `gridge_margin_krw` 에 접근 가능?
- [ ] Billing 이 AI Observer 의 프롬프트 원문에 접근 가능?
- [ ] `aiops_bridge_enabled = FALSE` 인데 bridge 파이프라인 실행?
- [ ] Anthropic 패스스루 플래그 불일치 (AI Observer 는 anthropic, Billing 은 일반)?
- [ ] 교차 이상 감지가 단일 제품의 데이터만으로 트리거?

## 참조

- AI Observer 데이터 모델: `products/aiops/schemas/INDEX.md`
