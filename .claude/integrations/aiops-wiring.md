# Integrations / AI Observer → Wiring — 규칙 본문

> I-001 본문. AI Observer 로그 데이터를 Wiring 대시보드에 노출하는 파이프라인.
> 통합 고객사 (Wiring + AI Observer 동시 사용) 대상. 로그/비용/거버넌스 단일 뷰.

---

## I-001 — AI Observer → Wiring 로그 파이프라인 (SHOULD)

### 전제

- 통합 고객사: Wiring 도입 후 AI Observer 활성화한 조직
- Wiring의 "보고서" / "로그" / "비용 관리" 탭에서 AI Observer 데이터 함께 표시
- AI Observer 단독 고객에게는 Wiring 연동 탭 자체 숨김
...
```sql
-- Wiring org 와 AI Observer org 는 동일 UUID 공유 (서로 다른 org 불허)
...
Wiring / AI Observer 의 Mode 설정이 **일치해야 함**:
- Wiring Mode A → AI Observer Mode A (SaaS)
- Wiring Mode B → AI Observer 온프레미스 (PA-011)
...
- Wiring 의 `hitl_cards` / `rule_timeline` 이 AI Observer 로 흘러가면 안 됨 (역방향 I-003 참조)
- AI Observer 의 `api_token` 은 Wiring UI 에 **원문 노출 절대 금지** (G-150)
...
- AI Observer 로그 적재 시 Wiring 로그 탭에 즉시 반영
...
// AI Observer → Wiring 이벤트 발행 (동일 org_id)
...
- 주간 성숙도 / 비용 집계는 배치 처리
- Wiring 의 "보고서" 탭이 AI Observer API 호출 (Server-side cached)
...
- 주간 보고서에 AI Observer 섹션 추가
- "AI 도구 사용 현황" / "채널별 비율" / "팀별 성숙도"
- 단, AI Observer 미활성 조직은 섹션 숨김

### 로그 탭

- 기존 Wiring 로그 (AI 에이전트 활동) + AI Observer 로그 (외부 AI 도구 사용) 양쪽 표시
- 필터: `source: wiring | aiops | all`

### 비용 관리 탭

- Wiring 비용 (LucaPus 에이전트) + AI Observer 비용 (직원 개인 AI 도구) 통합
- Mode별 분기 (PW-011 정합)
...
| Wiring 위계 | AI Observer 권한 | 볼 수 있는 AI Observer 데이터 |
|---|---|---|
...
- 사용자가 Wiring 로그인 → AI Observer 자동 로그인
...
AI Observer PA-007 에서 감지된 PII → Wiring 알림 인박스:
...
- 벨 아이콘 배지 + 인박스 카드
- 카드 클릭 → AI Observer 대시보드로 이동 (컨텍스트 유지)

### 민감 내용 노출 원칙

- Wiring 알림 카드에는 **pii_type만 표시** (실제 값 X)
- 상세 조회는 AI Observer 대시보드에서 (권한 재확인)
...
Mode B 통합 조직:
- **서로 다른 물리 인프라 가능** (Wiring = 고객 서버 A, AI Observer = 고객 서버 B)
- 이벤트 전달은 내부 네트워크 경유만 (Gridge 서버 경유 금지)
- 통합 UI 는 Wiring 인스턴스에서 AI Observer 인스턴스 API 직접 호출
...
### AI Observer → Wiring 전달 실패

- 이벤트 큐에 보관 (Redis Streams)
- 3회 재시도 + exponential backoff
- 지속 실패 시 OA 알림 + Wiring 에서 "AI Observer 연동 장애" 배너

### 과거 이벤트 복구

- Wiring 이 AI Observer API 로 `since=<timestamp>` 호출
- 놓친 이벤트 일괄 재수신
...
Wiring UI 에 AI Observer 내부 용어 노출 금지:
- ❌ "AI 옵저버" (내부 이름)
- ❌ "proxy.gridge.ai" 같은 내부 도메인
- ❌ `prompt_storage` / `admin_teams` 같은 DB 필드명

허용:
- "AI Observer 로그" / "AI 사용 현황"
- "AI Observer 대시보드로 이동"
...
- [ ] Wiring org_id ≠ AI Observer org_id 상태로 통합 시도?
- [ ] AI Observer `api_token` 이 Wiring UI 에 평문 노출?
- [ ] Mode B 에서 Gridge 서버 경유 이벤트 전달?
- [ ] L4 사용자에게 타인의 AI Observer 로그 노출?
- [ ] PII 원문이 Wiring 알림 카드에 표시?
- [ ] SSO 세션 공유 안 됨 (이중 로그인 요구)?
- [ ] 모드 불일치 (Wiring A + AI Observer B) 상태로 작동?
- [ ] AI Observer 내부 용어 Wiring UI 노출?

---

## 참조

- AI Observer 데이터 모델: `products/aiops/rules/data_model.md` (PA-001)
- AI Observer 권한: `products/aiops/rules/auth.md` (PA-004)
- AI Observer PII 감지: `products/aiops/rules/governance.md` (PA-007)
- AI Observer 이상 감지: `products/aiops/rules/alerts.md` (PA-009)
- AI Observer 성숙도: `products/aiops/rules/maturity.md` (PA-010)
