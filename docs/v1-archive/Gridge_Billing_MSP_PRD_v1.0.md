...
| AI 관측 도구 | (AI Observer) | 같은 고객에게 패키지 (Billing → AI Observer 업셀) |

**핵심 전략**: Billing → AI Observer / Wiring 으로 **번들 전환 20%+ / 10%+** 달성. 크레딧백 6개월 = CSM 릴레이션 구축 기간.
...
문서·UI·코드 어느 위치에서든 동일 의미로 해석되도록 호칭을 고정한다. 혼용 금지.

| 호칭 | 동의어 | 의미 | 권한 범위 |
|---|---|---|---|
| **슈퍼어드민** | 그릿지 어드민, Super, Gridge Admin | 그릿지가 고객사를 운영·관리하는 어드민 (위버·Phase 1+ Finance 등) | 전 고객사 |
| **고객 어드민** | Owner, Org Admin (별도 표기 없는 모든 "어드민"은 이쪽) | 고객사가 직접 관리하는 고객 Org의 최상단 어드민 | 자기 Org 전체 |
| **고객 팀 어드민** | Team Admin | 고객 어드민 하위의, 고객 Org 내 팀별 어드민 | 자기 팀만 |

**해석 규칙**
- 문서 본문에서 단순히 "어드민"이라고 적혀 있으면 = 고객 어드민
- 그릿지 측 어드민을 가리킬 때는 반드시 "슈퍼어드민" 또는 "그릿지 어드민"으로 명시
- 코드 안 식별자: `super_admin` · `org_admin` · `team_admin` (snake_case 고정)
...
- **플로우**:
  ...
  2. 02:00 교차 검증 (AI Observer ↔ Billing, I-004)
...
- **UI**: `/app/billing/creditback` 6개월 진행바 + D-30 경고 배너 + Wiring / AI Observer 업셀 연계
...
- **스키마 분리**: `billing.*` (AI Observer `aiops.*` 와 물리적 분리 G-091-06)
...
- 감사 로그 immutable 강제
- `G-091 Mode D` 직교 축 격리 (Wiring / AI Observer 와 분리)
...
- [ ] Billing → AI Observer / Wiring 번들 전환 자동 (I-005 파이프라인 고도화)
...
**Q3. 크레딧백 종료 후 고객 이탈은?**  
A. 6개월간 CSM 릴레이션 구축 + Billing → AI Observer / Wiring 업셀 전환 (20% / 10% 목표). 크레딧백 D-30 경고 + Luna 월간 리뷰에서 자연스러운 전환 제안.
