...
- **고객 포털**: https://billing-blond-alpha.vercel.app/login
- Mock 계정으로 즉시 접속 가능 (아래 [데모 계정](#데모-계정-mock-모드-전용) 표 참조)
- 운영 콘솔 / 직접 URL 로그인은 로컬 `npm run demo` 환경 권장

> Supabase 실 연동 여부는 배포 환경 변수 설정에 따라 다름. Mock 모드 배포 시 DB 쓰기는 동작하지 않음.
